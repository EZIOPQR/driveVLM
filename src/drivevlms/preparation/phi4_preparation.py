...
def prepare_model_and_processor_phi4(config):
    ...
    # tune lora only; freeze vision encoder.
    # On Ada (RTX 4090) GPUs, bf16 backward through SiglipEncoder + gradient_checkpointing
    # produces Inf/NaN grads on the very first step. Blackwell (5090) handles it fine.
    # Vision-LoRA (LoRA on the LM trunk) is enough for DriveLM SFT.
